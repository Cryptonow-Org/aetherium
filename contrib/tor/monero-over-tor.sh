...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/aetheriumd"
  then
    aetheriumd="$dir/aetheriumd"
    break
  fi
done
if test -z "$aetheriumd"
then
  echo "aetheriumd not found"
  exit 1
fi
echo "Found: $aetheriumd"

TORDIR="$DIR/aetherium-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting aetheriumd"
  tail -n 400 "$HOME/.bitaetherium/bitaetherium.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Aetherium\ \'" | tail -n 20
  exit 1
fi
