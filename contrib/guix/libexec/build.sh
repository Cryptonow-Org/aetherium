...
# Copyright (c) 2022-2024 The Aetherium Project
...
DISTNAME="aetherium-${HOST}-${VERSION}"
...
# target, so we don't spend time building Aetherium binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/aetherium-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,aetherium-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Aetherium build for HOST will be
