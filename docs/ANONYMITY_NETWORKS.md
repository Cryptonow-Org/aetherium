# Anonymity Networks with Aetherium

Currently only Tor and I2P have been integrated into Aetherium. The usage of
...
is made or until `aetheriumd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `aetherium-wallet-cli` and
`aetherium-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `aetheriumd` the ip/port of a socks proxy provided by a
...
which tells `aetheriumd` that ".onion" P2P addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `aetheriumd` the inbound address, network
...
which tells `aetheriumd` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:28083" and forwarded to `aetheriumd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `aetheriumd` localhost port 30000.
...
`aetheriumd` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/aetherium
...
This will store key information in `/var/lib/tor/data/aetherium` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/aetherium/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Aetherium network should have
similar timestamps.
...
### Intermittent Aetherium Syncing

If a user only runs `aetheriumd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `aetheriumd` as often as possible to conceal when transactions are being sent.
...
`aetheriumd` currently selects two outgoing connections every 5 minutes for
...
`aetheriumd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `aetheriumd` should likely disconnect
