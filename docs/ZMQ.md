# The Current/Future Status of ZMQ in Aetherium

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Aetherium
...
matching supported by both Aetherium and ZMQ. The `format`, `context` and `event`
...
The Aetherium daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Aetherium daemon also ensures that
...
The Aetherium daemon will send a `txpool_add` pub exactly once for each
