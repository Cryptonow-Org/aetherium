...
Core tests take longer than any other Aetherium tests, due to the high amount of computational work involved in validating core components.
...
To run only Aetherium's core tests (after building):
...
To run only Aetherium's crypto tests (after building):
...
```bash
aetheriumd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `aetherium-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Aetherium's hash tests (after building):
...
To run only Aetherium's performance tests (after building):
...
To run only Aetherium's unit tests (after building):
